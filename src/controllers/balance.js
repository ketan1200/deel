...
async function addBalance(req, res) {
  ...
  try {
    ...
    if (depositAmountReq > allowedDepositAmount) return res.status(400).send({ message: `Allowed balance limit ${Math.floor(allowedDepositAmount)}, Can't deposit more than it` });
